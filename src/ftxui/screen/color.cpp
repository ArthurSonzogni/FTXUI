...
std::string Color::Print(bool is_background_color) const {
  switch (color.channel.type_) {
    ...
    case ColorType::Palette256:
      return (is_background_color ? "48;5;"s : "38;5;"s) + std::to_string(color.channel.red_);

    case ColorType::TrueColor:
    default:
      return (is_background_color ? "48;2;"s : "38;2;"s)  //
             + std::to_string(color.channel.red_) + ";"                 //
             + std::to_string(color.channel.green_) + ";"               //
             + std::to_string(color.channel.blue_);                     //
  }
}
